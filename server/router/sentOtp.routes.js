...
export const sentOtp = async (req, res) => {
    try {
        ...
        await newUser.save().then(() => {
            res.status(201).send({
                message: `Hello ! ${fullName} ..!, Save successfully ..!`
            })
        }).catch((error) => {
            res.status(402).send({
        ...
        const mailOption = {
            ...
            text: `Hello ! ${userData.fullName} ..!, ${otp} Never Share your OTP, or Passcode with anyone, 
...
export const verifyOtp = async (req, res) => {
    try {
        ...
        if(userData.verifyOtpExpireAt > Date.now()){
            ...
            await newUser.save().then(() => {
                res.status(201).send({
                    message: `Hello ! ${fullName} ..!, email verified successfully ..!`
                })
            }).catch((error) => {
                res.status(402).send({
                    message: "Something went wrong ..!",
